...
function Homeroom() {
  ...
  useEffect(() => {
    api
      ...
      .then(({ data }) => setHomeroomViewTextContent(data))
      .catch((err) => {
  ...
  const handleFilterPosts = (selectedCategory) => {
    let newPostsToShow;
    newPostsToShow = selectedCategory !== 'All' ? homeroomPosts.filter((_) => _.attributes.post_title.trim()[0] === selectedCategory) : homeroomPosts;
    ...
    <>
      {homeroomViewTextContent.length ? (
        <div className="my-8" aria-label="media coverage page">
          <Section>
            <Section.Heading>{homeroomViewTextContent[0].attributes.section_heading}</Section.Heading>
            <Section.Text>{homeroomViewTextContent[0].attributes.section_description}</Section.Text>
