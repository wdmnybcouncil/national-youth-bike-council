...
function JoinUs() {
  ...
  const handleFilterFaqs = (selectedCategory) => {
    let newFaqsToShow;
    newFaqsToShow = (selectedCategory !== 'All')
      ? faqs.filter((_) => _.attributes.faq_title === selectedCategory)
      : faqs;
  ...
  const renderFaqs = (faqs) =>
    faqs.map(({ heading, description }) =>
      <Accordion key={heading} title={heading} content={description} />);
