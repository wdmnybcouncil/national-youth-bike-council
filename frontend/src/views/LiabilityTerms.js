                ...
                <form className="py-4 flex flex-col gap-4">
                  <div className="box-border flex items-center">
                    <input
                      ...
                      required />
                    <label htmlFor="agree-to-liability" className="ml-2">I have read and agree to the all the liability terms of the council.</label>
                  </div>
                  <div className="box-border flex flex-col gap-4">
                    <label className="w-full max-w-sm p-8 bg-skin-fill-card-accent text-center text-skin-accent rounded">
                      {agreementCode}
                    </label>
                    <button
                      type="button"
                      className={`focus:ring-offset-0.5 inline-block max-w-fit transform rounded-full bg-skin-button-accent px-6 py-2 text-sm uppercase tracking-wide text-skin-muted drop-shadow-md transition hover:-translate-y-0.5 hover:bg-skin-button-accent-hover focus:outline-none focus:ring-1 focus:ring-white lg:text-base ${(!hasUserReadTerms) && `pointer-events-none bg-gray-300 text-gray-500`}`}
