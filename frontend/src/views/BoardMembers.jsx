...
function BoardMembers({ boardMembers = [], onCardClick }) {
  const [boardMembersViewTextContent, setBoardMembersViewTextContent] = useState([]);
  ...
  useEffect(() => {
    api
      ...
      .catch((err) => {
        console.log('Uh-oh! Error occurred while fetching the board members data from the server.');
        console.log(err);
      });
  }, []);

  const renderBoardMemberCards = () =>
    boardMembers.map(({ attributes }) => {
    ...
    <>
      {boardMembersViewTextContent.length ? (
        <div className="my-8" aria-label="board members page">
          ...
          <Section>
            <Section.Heading>{boardMembersViewTextContent[0].attributes.section_heading}</Section.Heading>
            <div className="flex flex-wrap gap-6">{boardMembers.length ? renderBoardMemberCards() : null}</div>
          </Section>
          {/* Section # 2 - How to Become a Board Member  */}
          <Section>
            <Section.Heading>{boardMembersViewTextContent[1].attributes.section_heading}</Section.Heading>
