  ...
  const renderAlumniCards = () =>
    alumniMembers.map((member, index) => {
      ...
      return (
        <img
          key={index}
          src={profile_image.data.attributes.url}
          alt="alumni profile"
          className="h-40 w-40 max-w-full rounded-full border-4 border-skin-accent object-cover object-center"
        />
      );
    });
    ...
    <>
      {councilMembersViewTextContent.length ? (
        <div className="my-8" aria-label="council members page">
          ...
          <Section>
            <Section.Heading>{councilMembersViewTextContent[0].attributes.section_heading}</Section.Heading>
            <div className="flex flex-wrap gap-6">{councilMembers.length ? renderCouncilMemberCards() : null}</div>
          </Section>
          {/* Section # 2 - Alumni */}
          <Section>
            <Section.Heading>{councilMembersViewTextContent[1].attributes.section_heading}</Section.Heading>
            <div className="flex flex-wrap justify-center gap-8 md:justify-evenly md:gap-0">{alumniMembers.length ? renderAlumniCards() : null}</div>
          </Section>
        </div>
      ) : null}
    </>
